...
for i in inputList:
	...
	threesDone = False
	for letter in i:
		if i.count(letter) == 2 and not twosDone:
			twos += 1 
			twosDone = True
		if i.count(letter) == 3 and not threesDone:
			threes += 1
			threesDone = True
print twos * threes		
...
for i in xrange(0, len(inputList)):
	for j in xrange(1 + skipId, len(inputList) - 1):
		count = 0
		for letterPos in xrange(0, len(inputList[i])):
			if inputList[i][letterPos] != inputList[j][letterPos]:
				count += 1
		if count == 1:
			print inputList[i]
			print inputList[j]
