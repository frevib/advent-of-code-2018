...
[1518-07-04 00:36] falls asleep"""
    ...
    val inputTest = """[1518-09-16 00:04] Guard #1789 begins shift
[1518-05-01 00:01] Guard #107 begins shift
[1518-05-01 00:26] falls asleep
[1518-05-01 00:50] wakes up
[1518-06-01 00:26] falls asleep
[1518-06-01 00:27] wakes up
[1518-07-01 00:26] falls asleep
[1518-07-01 00:27] wakes up
[1518-08-01 00:26] falls asleep
[1518-08-01 00:27] wakes up
[1518-11-02 00:01] Guard #107 begins shift
[1518-12-31 00:02] Guard #523 begins shift
[1518-12-31 00:03] Guard #523 begins shift"""
    val inputList = input.split("\n")

    val guardSleepMinutes = HashMap<Int, IntArray>()
    var currentId = 0
    var fallAsleepMinute = 0

    inputList.sortedBy {
        LocalDateTime.parse("20${it.split("]")[0].substring(3)}", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))
    }.forEach { it ->
        val split = it.split("] ")
        val text = split[1]
        ...
        when {
            text.startsWith("G") -> {
                currentId = text.split("#")[1].substringBefore(" ").toInt()
            }
            text.startsWith("f") -> {
                fallAsleepMinute = LocalDateTime.parse("20${dateString.substring(3)}", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")).minute
            }
            text.startsWith("w") -> {
                val wakeupMinute = LocalDateTime.parse("20${dateString.substring(3)}", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")).minute
                val minutes: IntArray = guardSleepMinutes.getOrDefault(currentId, IntArray(60) { i -> 0 })

                for (i in fallAsleepMinute until wakeupMinute) {
                    minutes[i]++
                }

                guardSleepMinutes[currentId] = minutes
            }
        }
    }

    var maxGuardIdValue = 0
    var guardId = 0

    guardSleepMinutes.forEach { it ->
        val value = it.value.foldRight(0) { total, next -> total + next}
        if (value  > maxGuardIdValue) {
            maxGuardIdValue = value
            guardId = it.key
        }
    }
    val maxGuardMinutes = guardSleepMinutes[guardId]
    val max = maxGuardMinutes?.max() ?: 0

    println("First star: ${maxGuardMinutes?.indexOf(max)?.times(guardId)}")



    var maxMinuteCount = 0
    var guardIdSecondStar = 0

    guardSleepMinutes.forEach { it->
        val maxMinuteCountGuard: Int = it.value.max()!!

        if (maxMinuteCountGuard  > maxMinuteCount) {
            maxMinuteCount = maxMinuteCountGuard
            guardIdSecondStar = it.key
        }
    }

    println("Second star: ${guardSleepMinutes.get(guardIdSecondStar)!!.indexOf(maxMinuteCount).times(guardIdSecondStar)}")
